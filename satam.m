...
f = [1 : 1 : 20]; %integer counter and increment for status assignment
ff = 20; % a number equal to the size of the sample population
fff = 2/3; % fraction of negatives for disease in the test population
g = [1 : 1 : 20]; %integer counter and increment for detection assignment
h = 1; %integer counter and increment for detection assignment under a given sensitivity and specificity
i = [1 : 1 : 20]; %integer counter and increment for prevalence
j = [1 : 1 : 20]; %integer counter and increment for sensitivity
k = [1 : 1 : 20]; %integer counter and increment for specificity
aff = 0; % number of affecteds assigned
nor = 0; % number of normals assigned
l = [1 : 1 : 401]; % go back to 22500
l = 1;
...
r = 6; %{"unorma"};
s = 7; %{"normal"};
t = [1 : 1 : 8000]; % replace with 3307800 when working
kounter = 0; %initial value of fractional counter and increment for values between 0.02 and 20
%kounter2 = 1;
sample = 20; % number of individuals tested in simulation
isample = 1/sample; % multiplicative inverse of sample
maxprev = 0.485; % maximum sample prevalence
minprev = 0.2; % minimum or initial sample prevalence
maxsens = 0.985; % set maximum test sensitivity
minsens = 0.7; % set minimum or initial test sensitivity
maxspec = 0.985; % set maximum test specificity
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
v = [[0.2	0.215	0.23	0.245	0.26	0.275	0.29	0.305	0.32	0.335	0.35	0.365	0.38	0.395	0.41	0.425	0.44	0.455	0.47	0.485] [0.7	0.715	0.73	0.745	0.76	0.775	0.79	0.805	0.82	0.835	0.85	0.865	0.88	0.895	0.91	0.925	0.94	0.955	0.97	0.985] [0.7	0.715	0.73	0.745	0.76	0.775	0.79	0.805	0.82	0.835	0.85	0.865	0.88	0.895	0.91	0.925	0.94	0.955	0.97	0.985]]
...
B = combvec (sens(j),(1 - spec(k)),prev(i));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%end loops that calculate and assign at 20 intervals
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%y = 1 - spect(l);
%z = prevt(l);
meshgrid(senst,1-spect,prevt)
mesh(senst,1-spect,prevt); %
...
%  preva(l) = [0.2 : 0.0002 : 0.4998];
%end
%function specl = speci(l)
%  speci(l) = [0.7 : 0.0002 : 0.9998];
%end
%function sensl = sensi(l)
% sensi(l) = [0.7 : 0.0002 : 0.9998];
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
